...
void* thread_func(void *p)
{
    ...
    while(1){
        ...
        //printf("%s\n", current.pathNow);
        while(1){
            tempPos = current_Pos[pDelete->new_fd];
            slot[tempPos].erase(pDelete->new_fd);
            slot[tempPos - 1].insert(pDelete->new_fd);
            current_Pos[pDelete->new_fd] = tempPos - 1;
    ...
    while(1){
        ...
        for(i = 0; i < fdReadyNum; i++){
            if(evs[i].data.fd == sfd){
                ...
                slot[pos].insert(new_fd);
                current_Pos[new_fd] = pos;
                pthread_mutex_unlock(&pq->mutex); 
